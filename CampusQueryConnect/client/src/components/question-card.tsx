...
import { Question, users, currentUser } from "@/lib/mock-data";
import { UserBadge } from "./user-badge";
import { MessageCircle, Share2, Send, MoreHorizontal, Bookmark, Star, Heart } from "lucide-react";
...
export function QuestionCard({ question }: QuestionCardProps) {
  ...
  return (
    <div className="bg-card border-b border-border pb-4 mb-2">
      {/* Header */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-1.5">
              <span className="font-bold text-sm text-foreground cursor-pointer hover:opacity-80">{author.name}</span>
        ...
        {isExpanded && (
          ...
          >
            {localAnswers.map((answer) => {
              const answerAuthor = users[answer.userId];
              return (
